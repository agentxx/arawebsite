<div n:class="'address-container', $meta->displaySocialIcons && count($meta->socialIcons) > 0 ? social-icons-displayed">
	...
	<div class="content">
		...
		<div class="address-row row-web">
            ...
            {if substr($meta->web, 0, 7) == "http://" || substr($meta->web, 0, 8) == "https://"}
            ...
            {else}
                <div class="address-data"><p>{if $meta->web}<a href="http://{$meta->web}" target="_blank" itemprop="url" {if $settings->addressWebNofollow}rel="nofollow"{/if}>{if $meta->webLinkLabel}{$meta->webLinkLabel}{else}{$meta->web}{/if}</a>{else}-{/if}</p></div>
            {/if}
		</div>
		{/if}
	</div>
