...
function altered_search($query) {
	if($query->is_admin) {
		// manage here all admin side queries and frontend ajax calls
...
function aitFrontendNotices(){
	...
	$notices = array(
		'user-registration-success' => array(
			...
			"type"	=> "success pu-registered",
			"msg"	=> __('Thank you for joining the Artist-Run Alliance!</br></br>A confirmation email was sent to you in order to continue the registration process.</br></br>Please make sure the email did not end up in your SPAM folder.', 'ait'),
		),
