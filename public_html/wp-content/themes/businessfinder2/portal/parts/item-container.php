		...
		<div class="item-thumbnail">
			<a href="{$item->permalink}">
				<div class="item-thumbnail-wrap">
					{if $item->hasImage}
					<img src="{imageUrl $item->imageUrl, width => $imgWidth, height => $imgHeight, crop => 0}" alt="{!$item->title}">
					{else}
					<img src="{imageUrl $noFeatured, width => $imgWidth, height => $imgHeight, crop => 0}" alt="{!$item->title}">
					{/if}
				</div>
...
{else}
	...
	{var $imgHeight    = !empty($imgHeight) ? $imgHeight : 200}
	{var $textRows     = !empty($textRows) ? $textRows : 3}
